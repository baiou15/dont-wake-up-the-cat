...
class hackthonfunction():
    ...
    def getFile(self):
        ...
        for i in range(29):
            ...
            # commandLine = 'aws cloudwatch get-metric-statistics --namespace CWAgent --metric-name ' + self.metric_name + ' --dimensions Name=Status,Value=' + self.status + ' --statistics Maximum --start-time 2018-07-'+start+'T00:00:00 --end-time 2018-07-'+end+'T00:00:00 --period 300 --profile eabprod'
            # commandLine = 'aws cloudwatch get-metric-statistics --namespace CWAgent --metric-name disk_used_percent --dimensions Name=InstanceId,Value=i-081ce81b0d81cebc6 --statistics Maximum --start-time 2018-07-'+start+'T00:00:00 --end-time 2018-07-'+end+'T00:00:00 --period 300 --profile eabprod'
            commandLine = 'aws cloudwatch get-metric-statistics --namespace CWAgent --metric-name disk_used_percent --dimensions --dimensions Name=InstanceId,Value=i-028ddedc7959823db Name=path,Value=/ Name=device,Value=nvme0n1p1 Name=fstype,Value=ext4 --statistics Maximum --start-time 2018-07-' + start + 'T00:00:00 --end-time 2018-07-' + end + 'T00:00:00 --period 300 --profile eabprod'
            print(commandLine)
