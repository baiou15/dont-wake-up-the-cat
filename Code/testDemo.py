...
def getDataFile():
    ...
    commandLines = [
        ...
        ('Disk',
         ...
         ' Name=path,Value=/ Name=device,Value=nvme0n1p1 Name=fstype,Value=ext4 --statistics Maximum --start-time ',
         ' --end-time ', ' --period 300 --profile eabprod'),
        ('Memory',
    ...
    saveToFile(startTime, endTime, commandLines, s3Statuses, metric_names, instances, 0)
    saveToFile(startTime, endTime, commandLines, metadataStatus, metric_names, instances, 1)
    saveToFile(startTime, endTime, commandLines, clusterImportStatus, metric_names, instances, 2)
...
def saveToFile(startTime, endTime, commandLines, statuses, metrics, instances, instanceNum):
            ...
            if len(statuses) > 2:
                if metric_name == 'Velocity-T2':
                    for key, value in alldata.items():
                        ...
                        if accelerateCal[-2]:
                            t1 = float(accelerateCal[-2])
                        else:
                            t1 = 0
                        if accelerateCal[-1]:
                            t2 = float(accelerateCal[-1])
                        else:
                            t2 = 0
                        alldata[key] = value + ',' + str(t1 - t2)
    ...
    pushToS3('panto.eab.archive.dev', 'hackathon/' + instanceName + "Result.csv", outputString)
